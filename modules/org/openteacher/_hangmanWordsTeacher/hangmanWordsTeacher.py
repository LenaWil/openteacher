...
class TeachLessonWidget(QtGui.QSplitter):
	def __init__(self, keyboardWidget, mm, *args, **kwargs):
		super(TeachLessonWidget, self).__init__(*args, **kwargs)
		
		self._mm = mm
		
		qhangman = self._mm.import_("qhangman")
		self.word = self._mm.import_("word")

		self.hangmanWidget = qhangman.HangmanWidget(self._mm, self)
		...
		leftLayout.addWidget(self.hangmanWidget)
	...
	def newItem(self, word):
		print word["answers"][0][0]
		self._currentWord = word
		self.hangmanWidget.word = self.word.Word(word["answers"][0][0]) #The first obligatory answer
		self.hangmanWidget.hgraph.update()
	...
	def retranslate(self):
		...
		self.hangmanWidget.wordLessonWordLabel.setText(_("Word:"))
...
class TeachWidget(QtGui.QStackedWidget):
	...
	def _startLesson(self):
		...
		self._lessonWidget.updateLessonType(self._lessonType)
	...
	def _newItem(self, item):
		...
		self._lessonWidget.hangmanWidget.questionLabel.setText(compose(item["questions"]))
		try:
			self._lessonWidget.newItem(item)
			try:
				labelString = ""
				for i in xrange(self._lessonWidget.hangmanWidget.word.length):
					labelString += '-'
				self._lessonWidget.hangmanWidget.wordLabel.setText(labelString)
			except AttributeError:
				pass
		except IndexError:
			pass
	...
	def _buildUi(self, keyboardWidget):
		...
		self._lessonWidget = TeachLessonWidget(keyboardWidget, self._mm)
...
class WordsTeacherModule(object):
	def __init__(self, moduleManager, *args, **kwargs):
		...
		self.type = "wordsTeacher" #FIXME: make it a "wordsHangman" and register it to the words lesson
		self.uses = (
