...
class AnswersHandler(object):
	...
	@cherrypy.expose
	def index(self, test_id, **kwargs):
		cursor.execute(
			"SELECT teacher_id FROM tests WHERE test_id=?",
			(test_id,)
		)
		if cursor.fetchone()[0] != self._authorizationHandler.id:
			cherrypy.response.status = 401
			return json.dumps({"error": "unauthorized"})
		...
		if student_id:
			...
			return cursor.fetchone()[0] #list
		else:
			cursor.execute(
				"SELECT list FROM answers WHERE test_id=?",
				(test_id,)
			)
			return map(lambda result: result[0], cursor) #result[0] == list
